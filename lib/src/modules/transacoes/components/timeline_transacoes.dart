...
class TimelineTransacoes extends StatelessWidget {
  ...
  Widget _buildTimeline(List<TransacaoModel> transacoesOrdenadas) {
    ...
    for (int diaIndex = 0; diaIndex < diasOrdenados.length; diaIndex++) {
      ...
      final isPrimeiroDia = diaIndex == 0;
  ...
  Widget _buildGrupoDia(
    ...
    return Column(
      children: [
        ...
        // Transações do dia (cada uma com sua bolinha)
        ...transacoes.asMap().entries.map((entry) {
          final index = entry.key;
          final transacao = entry.value;
          final isPrimeiraTransacao = isPrimeiroDia && index == 0;
          final isUltimaTransacao = isUltimoDia && index == transacoes.length - 1;

          return _buildTransacaoComTimeline(
            transacao,
            isPrimeiraTransacao,
            isUltimaTransacao,
          );
  ...
  /// 📱 TRANSAÇÃO COM TIMELINE - Bolinha alinhada com o card
  Widget _buildTransacaoComTimeline(
    ...
    return IntrinsicHeight(
      child: Row(
        ...
        children: [
          // Coluna da timeline (linha + bolinha) - 40px de largura
          SizedBox(
            width: 40,
            child: Column(
              children: [
                // Linha superior (conecta com transação anterior)
                if (!isFirst)
                  Container(
                    width: 2,
                    height: 30,
                    color: corTema.withAlpha(78),
                  ),

                // Bolinha do status
                _buildIconeStatus(transacao),

                // Linha inferior (conecta com próxima transação)
                if (!isLast)
                  Expanded(
                    child: Container(
                      width: 2,
          ...
          // Card da transação
          Expanded(
            child: _buildTransacaoTimeline(transacao),
          ),
        ],
      ),
    );
            ...
            if (mostrarSaldoCorrente) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: corTema.withAlpha(78)),
                ),
                child: Row(
  ...
  Widget _buildChipRecorrente(String? tipoRecorrencia) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.repeat, size: 10, color: Colors.white),
          const SizedBox(width: 4),
          Text(
  ...
  Widget _buildChipParcelado(int parcelaAtual, int totalParcelas) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.credit_card, size: 10, color: Colors.white),
          const SizedBox(width: 4),
          Text(
