  ...
  Future<ResultadoEdicao<bool>> efetivar(
    ...
    try {
      ...
      if (!incluirFuturas || (transacao.grupoRecorrencia == null && transacao.grupoParcelamento == null)) {
        ...
        if (isOnline) {
          // 🌐 MODO ONLINE: Supabase é a fonte da verdade
          // ✅ Aguarda sync completar para garantir saldos corretos
          // ✅ Triggers do Supabase recalculam saldos automaticamente
          // ✅ Sync traz saldos corretos de volta para SQLite
          log('🌐 ONLINE: Aguardando sync para atualizar saldos...');
          await _syncManager.syncAll(); // AWAIT - espera sync terminar
          log('✅ Sync concluído - saldos atualizados do Supabase');

          // ⏱️ Delay generoso para GARANTIR que SQLite processou TUDO
          await Future.delayed(const Duration(seconds: 3));
          log('✅ Delay concluído - SQLite deve estar atualizado');
        } else {
          // 📱 MODO OFFLINE: Delta local para feedback imediato
          // ✅ Aplica alteração localmente para usuário ver mudança
          // ✅ Quando voltar online, sync corrigirá se necessário
          log('📱 OFFLINE: Aplicando delta local para feedback imediato...');
          if (transacao.contaId != null) {
            double delta = 0.0;
            if (transacao.tipo == 'receita') {
              delta = transacao.valor; // Receita aumenta saldo
            } else if (transacao.tipo == 'despesa') {
              delta = -transacao.valor; // Despesa diminui saldo
            } else if (transacao.tipo == 'transferencia') {
              delta = -transacao.valor; // Conta origem perde dinheiro
            }
            await _localDb.aplicarDeltaSaldo(transacao.contaId!, delta);
          }

          // Transferência: atualizar conta destino também
          if (transacao.tipo == 'transferencia' && transacao.contaDestinoId != null) {
            await _localDb.aplicarDeltaSaldo(transacao.contaDestinoId!, transacao.valor);
          }
      ...
      int efetivadas = 0;
      final deltasPorConta = <String, double>{}; // Acumular deltas por conta
      ...
      for (final t in transacoesFuturas) {
        if (!t.efetivado) {
          ...
          // Calcular delta para cada conta afetada (usado apenas em modo OFFLINE)
          if (t.contaId != null) {
            double delta = 0.0;
            if (t.tipo == 'receita') {
              delta = t.valor;
            ...
            deltasPorConta[t.contaId!] = (deltasPorConta[t.contaId!] ?? 0.0) + delta;
          }

          // Transferência: conta destino
          if (t.tipo == 'transferencia' && t.contaDestinoId != null) {
            deltasPorConta[t.contaDestinoId!] = (deltasPorConta[t.contaDestinoId!] ?? 0.0) + t.valor;
          }
        }
      }
      ...
      if (isOnline) {
        // 🌐 MODO ONLINE: Supabase é a fonte da verdade
        log('🌐 ONLINE: Aguardando sync para atualizar saldos ($efetivadas transações)...');
        await _syncManager.syncAll(); // AWAIT - espera sync terminar
        log('✅ Sync concluído - saldos atualizados do Supabase');
        ...
        // 📱 MODO OFFLINE: Aplica deltas acumulados
        log('📱 OFFLINE: Aplicando deltas para $efetivadas transações...');
        for (final entry in deltasPorConta.entries) {
          await _localDb.aplicarDeltaSaldo(entry.key, entry.value);
        }
    ...
    try {
      ...
      if (!incluirFuturas || (transacao.grupoRecorrencia == null && transacao.grupoParcelamento == null)) {
        ...
        if (isOnline) {
          ...
          // 📱 MODO OFFLINE: Delta reverso para feedback imediato
          log('📱 OFFLINE: Aplicando delta reverso local...');
          if (transacao.contaId != null) {
            double delta = 0.0;
            if (transacao.tipo == 'receita') {
              delta = -transacao.valor; // Receita desefetivada diminui saldo
            } else if (transacao.tipo == 'despesa') {
              delta = transacao.valor; // Despesa desefetivada aumenta saldo
            } else if (transacao.tipo == 'transferencia') {
              delta = transacao.valor; // Conta origem recupera dinheiro
            }
            await _localDb.aplicarDeltaSaldo(transacao.contaId!, delta);
          }

          // Transferência: atualizar conta destino também (reverso)
          if (transacao.tipo == 'transferencia' && transacao.contaDestinoId != null) {
            await _localDb.aplicarDeltaSaldo(transacao.contaDestinoId!, -transacao.valor);
          }
      ...
      for (final t in transacoesFuturas) {
        if (t.efetivado) {
          ...
          // Calcular delta REVERSO para cada conta afetada (usado apenas em modo OFFLINE)
          if (t.contaId != null) {
            double delta = 0.0;
            if (t.tipo == 'receita') {
              delta = -t.valor; // Receita desefetivada diminui saldo
            } else if (t.tipo == 'despesa') {
              delta = t.valor; // Despesa desefetivada aumenta saldo
            } else if (t.tipo == 'transferencia') {
              delta = t.valor; // Conta origem recupera dinheiro
            }
            deltasPorConta[t.contaId!] = (deltasPorConta[t.contaId!] ?? 0.0) + delta;
          }

          // Transferência: conta destino (reverso)
          if (t.tipo == 'transferencia' && t.contaDestinoId != null) {
            deltasPorConta[t.contaDestinoId!] = (deltasPorConta[t.contaDestinoId!] ?? 0.0) - t.valor;
          }
        }
      ...
      if (isOnline) {
        // 🌐 MODO ONLINE: Supabase é a fonte da verdade
        log('🌐 ONLINE: Aguardando sync para atualizar saldos ($desefetivadas transações)...');
        await _syncManager.syncAll(); // AWAIT - espera sync terminar
        log('✅ Sync concluído - saldos atualizados do Supabase');
        ...
        // 📱 MODO OFFLINE: Aplica deltas reversos acumulados
        log('📱 OFFLINE: Aplicando deltas reversos para $desefetivadas transações...');
        for (final entry in deltasPorConta.entries) {
          await _localDb.aplicarDeltaSaldo(entry.key, entry.value);
        }
  ...
  Future<void> _deleteTransacao(String id) async {
    ...
    if (isOnline) {
      try {
        ...
        // ✅ SINCRONIZAÇÃO BIDIRECIONAL após exclusão
        // Garante que Supabase e SQLite fiquem consistentes
        try {
          log('🔄 Forçando sincronização bidirecional após exclusão...');
          await SyncManager.instance.syncAll();
          log('✅ Sincronização bidirecional concluída');
        } catch (syncError) {
