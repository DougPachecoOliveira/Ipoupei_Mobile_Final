...
class _CategoriasSugeridasPageState extends State<CategoriasSugeridasPage> {
  ...
  Future<void> _carregarCategoriasSugeridas() async {
    ...
    try {
      ...
      final result = await _buscarCategoriasExistentes();

      // Criar lista de categorias sugeridas
      _categorias = _obterTodasCategoriasSugeridas(result['nomes']!, result['categorias']!);
  ...
  Future<Map<String, dynamic>> _buscarCategoriasExistentes() async {
    try {
      final categoriasExistentes = <String>{};
      final categoriasMap = <String, Map<String, dynamic>>{};
      ...
      for (final cat in [...categoriasReceitas, ...categoriasDespesas]) {
        ...
        try {
          ...
          // Adicionar subcategorias ao conjunto
          for (final sub in subcategorias) {
            final chaveSubcat = '${nome}_${sub.nome.toLowerCase()}';
  ...
  List<CategoriaComSubcategorias> _obterTodasCategoriasSugeridas(
    ...
    for (final catData in CategoriasSugeridasService.receitas) {
      categorias.add(_criarCategoriaComSubcategorias(catData, 'receita', nomesExistentes, categoriasMap));
    }

    // Adicionar DESPESAS
    for (final catData in CategoriasSugeridasService.despesas) {
      categorias.add(_criarCategoriaComSubcategorias(catData, 'despesa', nomesExistentes, categoriasMap));
    }
  ...
  CategoriaComSubcategorias _criarCategoriaComSubcategorias(
    ...
    // Buscar dados da categoria existente
    final categoriaExistente = categoriasMap[nomeCategoria];
    final categoriaExistenteId = categoriaExistente?['id'] as String?;
    final subcategoriasExistentes = categoriaExistente?['subcategorias'] as List<SubcategoriaModel>?;
    ...
    return CategoriaComSubcategorias(
      ...
      categoriaExistenteId: categoriaExistenteId,
    ...
    try {
      ...
      for (final categoria in _categorias) {
        ...
        try {
          ...
          if (categoria.jaExiste) {
            // Usar ID da categoria existente
            categoriaId = categoria.categoriaExistenteId ?? '';
            debugPrint('ℹ️ Categoria ${categoria.nome} já existe com ID: $categoriaId');
          } else {
            ...
            debugPrint('✅ Categoria ${categoria.nome} criada com ID: $categoriaId');
          }

          // Criar subcategorias selecionadas (apenas as que não existem)
          for (int i = 0; i < categoria.subcategorias.length; i++) {
            ...
            final subcategoriaJaExiste = (categoria.subcategoriasExistentes != null)
                ? categoria.subcategoriasExistentes!.any(
                    (sub) => sub.nome.toLowerCase() == nomeSubcategoria.toLowerCase()
                  )
                : false;

            if (subcategoriaJaExiste) {
              debugPrint('ℹ️ Subcategoria $nomeSubcategoria já existe, pulando...');
              continue;
            }
          ...
          // Contar categoria como importada se criou ela OU adicionou subcategorias
          if (!categoriaCriada && subcategoriasImportadas > 0) {
            categoriasImportadas++;
          }

        } catch (e) {
          debugPrint('❌ Erro ao processar categoria ${categoria.nome}: $e');
  ...
  Widget _buildSectionHeader() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.tealPrimary.withAlpha(52)),
      ),
      child: Column(
        children: [
          const Row(
  ...
  Widget _buildCategoriaCard(CategoriaComSubcategorias categoria, int index) {
    ...
    if (jaImportada && temSubcategoriasDisponiveis) {
      ...
      bordaCor = Colors.green.withAlpha(182);
      sombraCor = Colors.green.withAlpha(38);
      larguraBorda = 1.5;
    } else if (jaImportada) {
      // Categoria completamente importada
      bordaCor = Colors.green.withAlpha(104);
      sombraCor = Colors.green.withAlpha(26);
      larguraBorda = 1;
    } else if (categoria.selecionada) {
      // Categoria nova selecionada
      bordaCor = cor;
      sombraCor = cor.withAlpha(52);
      larguraBorda = 2;
    } else {
      // Categoria padrão
      bordaCor = AppColors.cinzaBorda.withAlpha(78);
      sombraCor = Colors.black.withAlpha(21);
      larguraBorda = 1;
    }
  ...
  Widget _buildBadgeJaExiste() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.cinzaMedio.withAlpha(52),
        borderRadius: BorderRadius.circular(12),
      ),
      child: const Text(
        'Já existe',
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: subcategoriaJaExiste
                ? Colors.green.withAlpha(26) // Verde suave para já existente
                : selecionado
                    ? cor.withAlpha(38)
                    : Colors.white,
            border: Border.all(
              color: subcategoriaJaExiste
                  ...
                  : selecionado
                      ? cor
                      : AppColors.cinzaBorda.withAlpha(130),
...
class CategoriaComSubcategorias {
  ...
  final String? categoriaExistenteId; // ID da categoria existente (se jaExiste = true)
  final List<SubcategoriaModel>? subcategoriasExistentes; // Lista de subcategorias existentes
  bool selecionada;
  ...
  CategoriaComSubcategorias({
    ...
    this.categoriaExistenteId,
    this.subcategoriasExistentes,
  ...
  /// Quantas subcategorias já existem (contador)
  int get countSubcategoriasExistentes => subcategoriasExistem.where((e) => e).length;
}