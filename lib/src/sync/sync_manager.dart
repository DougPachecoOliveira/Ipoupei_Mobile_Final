...
class SyncManager {
  ...
  Future<void> _downloadTransactionsFullRange(String userId) async {
    try {
      ...
      debugPrint('📦 ${transactions.length} transações encontradas no range completo (SUPABASE = FONTE DA VERDADE)');
      ...
      debugPrint('💾 SQLite ANTES: ${sqliteBefore?.first['total']} transações locais');
      ...
      debugPrint('💾 SQLite DEPOIS: ${sqliteAfter?.first['total']} transações locais');
      debugPrint('✅ Sincronização: $novos novos + $atualizados atualizados = ${transactions.length} total');
  ...
  Future<void> limparFilaCorrempida() async {
    try {
      ...
      final deletedCount = await _localDB.database?.delete(
        'sync_queue',
        where: "record_id LIKE 'temp_%' OR record_id LIKE '%_virtual%' OR record_id LIKE '%_rpc%'",
      );

      debugPrint('🗑️ Fila limpa: $deletedCount itens com IDs temporários removidos');
  ...
  Future<void> _uploadUpdate(String tableName, String recordId) async {
    ...
    try {
      ...
      if (tableName == 'transacoes' && record['conta_id'] != null) {
        debugPrint('🔥 Forçando recálculo de saldo no Supabase para conta ${record['conta_id']}...');
        try {
  ...
  Future<void> _uploadUpsert(String tableName, String recordId) async {
    ...
    try {
      ...
      if (recordId.startsWith('temp_') && tableName != 'planejamentos') {
        debugPrint('⚠️ Pulando ID temporário para tabela $tableName: $recordId');
  ...
  Future<void> _uploadPlanejamentoUpsert(Map<String, dynamic> record) async {
    ...
    try {
      ...
      final response = await Supabase.instance.client.rpc('ip_prod_upsert_planejamento', params: params);

      debugPrint('✅ RESPOSTA SUPABASE: $response');
      debugPrint('✅ Tipo da resposta: ${response.runtimeType}');

      // Verificar se a resposta indica erro
      if (response != null && response.toString().toLowerCase().contains('error')) {
        throw Exception('RPC retornou erro: $response');
      }

      if (response == null) {
        debugPrint('⚠️ ATENÇÃO: RPC retornou null - pode indicar sucesso ou erro silencioso');
      }
  ...
  Future<void> _downloadCartoes(String userId) async {
    try {
      ...
      debugPrint('📦 Cartões ${lastSync != null ? "modificados" : "encontrados"}: ${cartoes.length}');
  ...
  Future<void> _downloadUserCategories(String userId) async {
    try {
      ...
      debugPrint('📦 Categorias ${lastSync != null ? "modificadas" : "encontradas"}: ${categories.length}');
  ...
  Future<bool> _checkIfTableNeedsSync(String tableName, String userId) async {
    try {
      ...
      if (latestUpdateDate.isAfter(lastSyncDate)) {
        debugPrint('🔄 $tableName tem mudanças: servidor=$latestUpdate vs local=$lastSync');
        return true;
      }
  ...
  Future<void> syncPerfilUsuario({bool force = false}) async {
    ...
    // Verificar se já fez sync recentemente (apenas se não for forçado)
    final now = DateTime.now();
    if (!force && _lastPerfilUsuarioSync != null &&
        now.difference(_lastPerfilUsuarioSync!) < _syncCooldown) {
      debugPrint('⏩ Sync de perfil_usuario pulado - muito recente');
      return;
    }

    debugPrint(force ? '🔄 FORÇANDO sync de perfil_usuario...' : '🔄 Sync normal de perfil_usuario...');
      ...
      if (response is List) {
        ...
        for (final item in response) {
          ...
          if (existing.isEmpty) {
            ...
            final saldoAntigoSQLite = (existing.first['saldo'] as num?)?.toDouble();
            ...
            if (saldoAntigoSQLite != saldoNovoSupabase) {
              debugPrint('     ⚠️ DIFERENÇA DETECTADA: Δ R\$ ${((saldoNovoSupabase as num).toDouble() - saldoAntigoSQLite!).toStringAsFixed(2)}');
            }
            await _localDB.database!.update(
              'contas',
  ...
  Future<void> _downloadPlanejamentos(String userId) async {
    try {
      ...
      if (lastSync != null) {
        query = query.gte('updated_at', lastSync);
        debugPrint('🔄 Sync incremental de planejamentos: apenas após $lastSync');
      }

      final planejamentos = await query.order('created_at');

      debugPrint('📊 ${planejamentos.length} planejamentos ${lastSync != null ? "modificados" : "encontrados"}');
